...
const CoolColors: string[] = ['#00c853','#00e676','#69f0ae','#76ff03','#eeff41','#c6ff00','#ffea00','#ffff00','#ffc400','#ff8f00','#ffa000','#ff6d00','#e65100','#ffd180','#ff3d00','#ff9e80','#a7ffeb','#64ffda','#1de9b6','#00bfa5','#00e5ff','#00b8d4','#0091ea','#82b1ff','#2962ff','#304ffe','#3d5afe','#7c4dff','#651fff','#6200ea','#aa00ff','#d500f9','#e040fb','#ea80fc','#c51162','#f50057','#ff4081','#ff1744',
]
...
export class ChatboxComponent implements OnInit {
  ...
  private room$!: Subscription;
	constructor(private socketIoService: SocketIoService, public dialog: MatDialog, private playerService: PlayerService) {}

	ngOnInit(): void {
	  this.getNewInfoSub$ = this.socketIoService.getNewInfo().subscribe((userTypeMessage: [string, string, string, string]) => {
			let newMessage: Info = {user: userTypeMessage[0], name: userTypeMessage[1], type: userTypeMessage[2], message: userTypeMessage[3]};
			this.infoListGlobal.push(newMessage);
		})

    this.getNewInfoRoomSub$ = this.socketIoService.getNewInfoRoom().subscribe((userTypeMessage: [string, string, string, string]) => {
			let newMessage: Info = {user: userTypeMessage[0], name: userTypeMessage[1], type: userTypeMessage[2], message: userTypeMessage[3]};
			this.infoListRoom.push(newMessage);
		})

    this.selfJoinRoomSub$ = this.socketIoService.selfGetRoom().subscribe((room: string) => {
      this.infoListRoom = [];
  ...
  leaveRooms() {
	...
	sendMessage() {
      ...
      this.socketIoService.sendMessageRoom(this.newMessage);
	  this.newMessage = '';
	}
