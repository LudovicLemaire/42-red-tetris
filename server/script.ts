...
interface ServerToClientEvents {
	...
	info_room: (user: string, name: string, type: string, message: string) => void;
	...
	get_room_members: (members: {id: string, name: string, isAdmin: boolean}[]) => void;
...
io.on("connection", async (socket) => {
	...
	socket.on('message_room', (message:string) => {
		if (self.room !== '' && self.room !== undefined)
			io.in(self.room).emit('info_room', `${socket.id}`, `${self.getAdminName()}`, 'message_room', `${message}`);
	});

	socket.on('rename', (name:string) => {
		io.emit('info', `${socket.id}`, `${self.name}`, 'rename', `${self.name} renamed himself as ${name}`);
		let oldName = self.name;
		self.name = name;
		if (self.room !== '' && self.room !== undefined) {
			io.in(self.room).emit('get_room_members', getRoomMembers(self.room));
			io.in(self.room).emit('info_room', `${socket.id}`, `${self.name}`, 'rename_room', `${oldName} renamed himself as ${self.name}`);
		}
	});
	...
	socket.on('join_room', async (room:string) => {
		...
		if (roomIndex !== -1) {
			...
			allGames.push(game);
			await joinRoom(room);
			io.emit('info', `${socket.id}`, `${self.name}`, 'create_room', `${self.name} created ${room} room`);
			self.isAdmin = true;
		...
		if (roomIndex !== -1) {
			for (const member of allGames[roomIndex].members) {
				if (member.id === id) {
					...
					const memberIndexToRemove = allGames[roomIndex].members.findIndex((element) => element.id === id);
					...
					if (allGames[roomIndex].members.length === 0) {
						const gameIndexToRemove = allGames.findIndex((element) => element.id === allGames[roomIndex].id);
						if (gameIndexToRemove > -1)
							allGames.splice(gameIndexToRemove, 1);
					}
					member.room = '';
					io.to(member.id).emit('kicked', true);
					io.to(member.id).emit('toast', 'warning', 'You have been jettisoned in space');
					io.to(member.id).emit('self_get_room', member.room);
					socket.emit('toast', 'info', 'You jettisoned ' + member.name);
					io.in(self.room).emit('info_room', `${self.id}`, `${self.name}`, 'leave_room', `${member.name} have been jettisoned in space`);
					io.in(self.room).emit('get_room_members', getRoomMembers(self.room));
					io.emit('get_games', getAllAvailableRoomsReduced())
				}
			}
		...
		if (roomIndex !== -1) {
			for (const member of allGames[roomIndex].members) {
				if (member.id === id) {
					member.isAdmin = true;
					self.isAdmin = false;
					io.to(member.id).emit('toast', 'show', 'Congratulations !</br>You have been promoted to Captain.');
					io.to(member.id).emit('self_is_admin', true);
					socket.emit('self_is_admin', false);
					socket.emit('toast', 'info', 'You convey your Captain\'s hat to ' + member.name);
					io.in(self.room).emit('info_room', `${self.id}`, `${member.name}`, 'new_admin_room', `🚀 ${member.name} has been promoted to Captain !`);
					io.in(self.room).emit('get_room_members', getRoomMembers(self.room));
				}
			}
		}
	...
	async function joinRoom(room: string) : Promise<void> {
		...
		socket.join(room);
		io.in(room).emit('info_room', `${socket.id}`, `${self.name}`, 'join_room', `${self.name} joined the room`);
		io.emit('get_games', getAllAvailableRoomsReduced());
	...
	async function leaveRooms(): Promise<void> {
		...
		for (const game of allGames) {
			if (game.id == self.room) {
				// delete user from game members
				const memberIndexToRemove = game.members.findIndex((element) => element.id === self.id);
				if (memberIndexToRemove > -1)
					game.members.splice(memberIndexToRemove, 1);
				// delete game if empty
				if (game.members.length === 0) {
					const gameIndexToRemove = allGames.findIndex((element) => element.id === game.id);
					if (gameIndexToRemove > -1)
						allGames.splice(gameIndexToRemove, 1);
				}
			}
		}
		io.in(self.room).emit('info_room', `${socket.id}`, `${self.name}`, 'leave_room', `${self.name} left the room`);
		// give new admin
		const oldRoomIndex = allGames.findIndex((element) => element.id === self.room);
		if (self.isAdmin && oldRoomIndex !== -1 && allGames.length > 0 && allGames[oldRoomIndex].members.length > 0) {
			allGames[oldRoomIndex].members[0].isAdmin = true;
			io.to(allGames[oldRoomIndex].members[0].id).emit('toast', 'show', 'Congratulations !</br>You have been promoted to Captain.');
			io.to(allGames[oldRoomIndex].members[0].id).emit('self_is_admin', true);
			io.in(self.room).emit('info_room', `${allGames[oldRoomIndex].members[0].id}`, `${allGames[oldRoomIndex].members[0].name}`, 'new_admin_room', `🚀 ${allGames[oldRoomIndex].members[0].name} has been promoted to Captain !`);
		}
		io.in(self.room).emit('get_room_members', getRoomMembers(self.room));
		self.isAdmin = false;
		self.room = '';
	...
	function getRoomMembers(room: string): {id: string, name: string, isAdmin: boolean}[] {
		let roomMembers: {id: string, name: string, isAdmin: boolean}[] = [];
		for (const game of allGames) {
			if (game.id == room) {
				for (const member of game.members) {
					roomMembers.push({id: member.id, name: member.name, isAdmin: member.isAdmin})
				}
			}
		}
		return roomMembers;
